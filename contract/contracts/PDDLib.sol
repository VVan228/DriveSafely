...
function randMod(uint _modulus,uint _crossId, uint _randNonce) internal pure returns(uint) {
...
function remove(uint[] array, uint index) external pure returns(uint[]) {
...
function isMainRoad(uint _mainRoad1, uint _mainRoad2, uint _road) external pure returns(bool){
    return _road==_mainRoad1 || _road==_mainRoad2;
}
function getNextDnaValue(uint _dna) internal returns(uint value, uint dna) external pure {
    value = _dna % 10; 
    dna = _dna / 10;
}
}